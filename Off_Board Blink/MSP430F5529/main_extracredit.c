...
void main()
{
    ...
    P2DIR = BIT3 + BIT4 + BIT5 + BIT6; //set 4 pins to be output. The + sign means OR
    P2OUT = BIT3 + BIT4 + BIT5 + BIT6; //turns on 4 pins and produces 3.3V
    
    P2REN = BIT1; //internal resistor is enabled, P2.1 connects to either Vcc (3.3V) or ground
    P2OUT = BIT1; //P1.3 is an input pin; this line defines internal resistor to connect to 3.3V
    ...
    while(1) //loop indefinitely
    {
        ...
        if(((P2IN & BIT1)!=BIT1) & x == 0) //logic AND. P2IN and BIT1 must be 1 for output to be 1
