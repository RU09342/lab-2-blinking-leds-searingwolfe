...
void main()
{
    ...
    P2DIR = BIT2 + BIT3 + BIT4 + BIT5; //set 4 pins to be output. The + sign means OR
    P2OUT = BIT2 + BIT3 + BIT4 + BIT5; //turns on 4 pins and produces 3.3V
    ...
    while(1) //loop indefinitely
    {
        ...
        { //line above means: if result of port 1 input AND P1.1 pin (ONE) is NOT equal to P1.1 (ONE), 
			...
			P2OUT ^= BIT2 + BIT3 + BIT4 + BIT5; //set  + the output of the pins. ^ means toggle
